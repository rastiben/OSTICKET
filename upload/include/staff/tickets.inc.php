...
case 'closed':
    $queue_columns['date']['heading'] = __('Date fermeture');
 ...
 <table class="list" border="0" cellspacing="1" cellpadding="2" width="100%">
    <thead>
     ...
     <tbody>
        ...
        'ticket_id', 'number', 'cdata__subject', 'user__default_email__address',
        'source', 'cdata__:priority__priority_color', 'cdata__:priority__priority_desc',
        'status_id', 'status__name', 'status__state', 'dept_id', 'dept__name',
        'user__name','user__cdata__firsname', 'lastupdate', 'isanswered', 'staff__firstname',
        'staff__lastname', 'team__name','user_id','user__org_name');
...
<?php               if ($T['attachment_count'])
                ...
                <td nowrap><div><?php
                    ...
                    $un = new UsersName($T['user__name']);
                        echo '<a href="./users.php?id='. $T['user_id'] .'#tickets">' . ucwords($T['user__cdata__firsname'].' '.$T['user__name']) . '</a>';
                    ?></span></div></td>
                <td nowrap><div><?php
                    if ($T['collab_count'])
                        echo '<span class="pull-right faded-more" data-toggle="tooltip" title="'
